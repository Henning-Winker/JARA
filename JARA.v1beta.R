...
if(abundance=="relative"){
  ...
  # Bundle data
  jags.data <- list(y = log(qI_y),SE2=qse2, logY1 = log(qI_y[1,1]), T = length(year),nI=n.indices,sigma.fixed=ifelse(sigma.proc==TRUE,0,sigma.proc),igamma=igamma,penSig=0)
  ...
  cat("  
      # Process variance prior
      isigma2.est ~ dgamma(igamma[1],igamma[2])
      pen.sigma <- ifelse(sigma>0.8,log(sigma)-log(0.8),0) 
...
jara.mod <- jags(jags.data, inits, parameters, "jara.jags", n.chains = nc, n.thin = max(nt,2), n.iter = max(ni/5,10000), n.burnin = nb/10)
...
png(file = paste0(output.dir,"/Fits_",assessment,".png"), width = 5, height = 4, 
    res = 200, units = "in")
par(Par)
...
Par = list(mfrow=c(1,1),mar = c(5, 5, 1, 1), mgp =c(3,1,0),mai = c(0.7, 0.7, 0.1, 0.1),mex=0.8, tck = -0.02,cex=0.7)
png(file = paste0(output.dir,"/PopTrend_",assessment,".png"), width = 5, height = 4, 
    res = 200, units = "in")
par(Par)
...
lines(rep(year[mp.assess[1]],2),c(0,m2),lty=2)
...
#### IUCN plot
Par = list(mfrow=c(1,1),mar = c(5, 5, 1, 1), mgp =c(3,1,0),mai = c(0.7, 0.7, 0.1, 0.1),mex=0.8, tck = -0.02,cex=0.7)
png(file = paste0(output.dir,"/IUCNplot_",assessment,".png"), width = 4, height = 4, 
    res = 200, units = "in")
par(Par)
change = (apply(posteriors$Ntot[,(mp.assess[2]-1):(mp.assess[2]+1)],1,mean)/apply(posteriors$Ntot[,(mp.assess[1]-1):(mp.assess[1]+1)],1,mean)-1)*100
den = density(change)
x1 = den$x
y1 = den$y
